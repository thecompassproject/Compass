...
public:
// (3): input or initialise initial parameters like masses, inital angular coordinates, etc
	void input();
	double acceleration1(double,double,double,double);
	double acceleration2(double,double,double,double);
	void calc();
};
...
void double_pendulum::input() {
	...
	cout << "Enter the lengths length1 and length2::\n";
	cin >> length1 >> length2;

	cout << "Enter the angles angle1 and angle2::\n";
	cin >> angle1 >> angle2;

	angle1 = angle1 * (M_PI / 180);  // convert the angles into radians
	angle2 = angle2 * (M_PI / 180);  // from degrees.
...
double double_pendulum::acceleration2(double theta1, double theta2, double omega1, double omega2) {
	return (2*sin(theta1 - theta2)*(omega1*omega1*length1*(mass1 + mass2) + g*(mass1 + mass2)*cos(theta1) + omega2*omega2*length2*mass2*cos(theta1 - theta2))) / (length2 * (2*mass1 + mass2 - mass2*cos(2*theta1 - 2*theta2)));
}
...
void double_pendulum::calc() {
	...
	double h = 0.0005;    //define a step size
...
for(double i = 0 ; i < 20000 ; i++) {
		...
		angular_velocity2 = angular_velocity2 + ((1.0/3.0)*f1 + (2.0/3.0)*f2) ;

		angle1 = angle1 + angular_velocity1 * h;
		angle2 = angle2 + angular_velocity2 * h;
